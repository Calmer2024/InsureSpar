...
【指代消解与容错原则】请结合完整的【对话历史】推断这笔报价真实的被保人是谁（如丈夫、妻子或孩子）。如果对话历史中确实缺失年龄、保额或交费期等参数，**必须填入 null**，绝对不可强行套用客户本人的静态画像参数！"""
    ...
    user_prompt = f"""【对话历史 (作为上文背景)】
{conversation_history_text}

【待核查的本轮销售发言】
由于你只核查销售是否“在这句话里”说了假事实：
销售说："{sales_msg}"

【客户刚给出的最新回复】
客户回应："{customer_reply}"

请仅从“销售说”抽取出所有的事实性声明。不要把本轮客户的话当作上一步客户的问题！"""
...
1. 熔断法则 1（数字造假）：如果在比较“考官独立事实核查报告”与“销售发言”时，发现销售给出的保费、现金价值计算出错，导致专业性得分(professionalism_score) ≤ 6，则必须惩罚误导行为，强制令合规性得分(compliance_score) ≤ 6。
...
4. 如果销售的回答完美，可给出一种维度扣一分甚至全部满分的评价。

【策略灵活性原则】
[教练旁白] 仅作参考。只要销售使用了合理有效的沟通策略推进了对话，即使没听教练的话也**不得扣减策略分**。只有当销售完全回避客户核心痛点、或者话术逻辑混乱时才扣分。

【注意事项（关于时间线与评判标准）】
1. 绝对时间线：销售说完一句话，客户才给出了最新的回复。
2. 你是在**客户回复后**进行评价。因此，客户在本轮最新回复中提出的新问题或新要求，销售在本轮（前面那句话）是不可能预测并回答的！
3. 绝对禁止因为“销售本轮没有回答客户在最新回复中刚提出的新质疑”而扣掉专业分或策略分。
4. 你应该利用最新这句【客户反应】来：
   - 反推销售上一句（即本轮销售发言）的沟通策略是否起效（如果客户反应冷淡甚至反感，说明销售刚才的策略失败；如果客户愿意提供信息，说明策略成功）。
   - 在 `overall_advice` 中指导销售在**下一轮**该如何应对客户刚发出的最新回复。

【判卷维度 (0-10)】
1. 专业性：基于时间线，销售对【对话台账】里客户**上一轮**提问的回答用语是否准确专业。数据事实（保费等）与工具核查是否一致？造假≤3分。
2. 合规性：是否存在违背常理、误导、隐瞒的陈述？
3. 策略性：结合当前阶段，基于【客户反应】的反馈，销售上一步的破局策略是否得当？

输出必须包含以下7个JSON字段，分数必须是纯数字：
{{"professionalism_score":8,"compliance_score":9,"strategy_score":5,"professionalism_comment":"评价","compliance_comment":"评价","strategy_comment":"基于客户反应的评价","overall_advice":"具体、一针见血的建议，告诉销售下一句应对客户最新回复的思路，严格限制在50字以内"}}"""
...
【核心核查对象：本轮销售发言】
请仔细评判以下销售的发言（它是对【对话台账】中最后一条日志的回复）：
销售说：{sales_msg[:500]}
{sales_tool_evidence}
...
注意：利用这条反馈判断销售策略好坏，并在 overall_advice 给出下一步指引。切勿因销售没回答这条反馈里刚提出的问题而扣分！
